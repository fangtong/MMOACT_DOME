package com.CoreLib.Delegate
{
	import com.CoreLib.Delegate.Enum.EnumSoundLayer;
	import com.CoreLib.Delegate.Interface.ISoundDelegate;
	...
	public class SoundDelegate extends EventDispatcher implements ISoundDelegate
	{
		...
		public function SoundDelegate()
		{
			...
			for(var i:int = 0; i < EnumSoundLayer.ENUM_SOUNDLAYER_MAX; i++)
			{
		...
		public function AddSoundBySource(i_soundName:String, i_sound:SSSound,i_layer:uint):void
		{
			...
			if(i_layer >= EnumSoundLayer.ENUM_SOUNDLAYER_MAX) return;
		...
		public function PlaySound(i_soundName:String, startTime:Number=0, loops:int=0,inQueue:Boolean = false,sndTransform:SoundTransform=null):void
		{
			...
			//音乐 替换
			{
				var musics:Vector.<SSSound> = _layerSounds[EnumSoundLayer.ENUM_SOUNDLAYER_MUSIC];
				if(musics.indexOf(sound) != -1)
				{
					StopLayerSounds(EnumSoundLayer.ENUM_SOUNDLAYER_MUSIC);
				}
			}
			//音效 不替换
			//进入队列 等待播放
