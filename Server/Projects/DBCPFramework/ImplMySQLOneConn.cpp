...
namespace SevenSmile{
	namespace DBCPFramework{
		namespace ImplBehaviors{
			ImplMySQLOneConn::ImplMySQLOneConn(WaitRes i_bEnableWaitRes)
				:_lpMysql(0)
				,_strSQL(0)
				,_lpCharIPAddr(0)
				,_intPort(0)
			...
			ImplMySQLOneConn::~ImplMySQLOneConn(void)
			{
				if (0!=_strSQL)
				{
					free(_strSQL);
			...
			bool ImplMySQLOneConn::Close( void ) throw()
			{
				bool res=false;
				try
				{
			...
			void ImplMySQLOneConn::FreeRes( IResult<MYSQL_RES>* i_lpRes )
			{
				if (0==i_lpRes)
				{
					return;
			...
			void ImplMySQLOneConn::FreeResWithThreads( IResult<MYSQL_RES>* i_lpImplResult )
			{
				_opt=FREE_RES;
				_lpResultFree=i_lpImplResult;
			}

			void ImplMySQLOneConn::Query( const char* i_strQuery )
			{
				_opt=EXECUTE;
			...
			IResult<MYSQL_RES>* ImplMySQLOneConn::Execute(
			...
			unsigned int ImplMySQLOneConn::Do( void* i_lpVoidParams/*=0*/ )
			{
				if (EXECUTE==_opt)
				{
					...
					if (_bEnableWaitRes)	//要等待执行结果的
					{
						...
						if (_bNeedRes)
						{
							_lpResultTemp->Signal();
						}
					}
				}
				else if (FREE_RES==_opt)
				{
			...
			void ImplMySQLOneConn::SetConnectionPool( IConnectionPool<MYSQL_RES>* i_lpImplConnectionPool )
			{
				_lpConnectionPool=i_lpImplConnectionPool;
			}

			void ImplMySQLOneConn::SetAutoReconnect( bool i_bAutoReconnect )
			{
